...
def backtest(begin_date = None, end_date = None):
    """
    ...
    for _date in all_dates:
        ...
        if last_date is not None and len(before_sell_holding_codes) > 0:
            ...
            for current_daily in current_daily_cursor:
                # 当前交易日的复权因子
                # if not('au_factor' in current_daily):
                #     continue
                ...
                if code in code_last_aufactor_dict:
                    ...
                    # 价格的关系：last_close * last_aufactor = pre_close * current_aufactor = 前一个交易日的后复权价格
                    # 转换之后得到下面的公式：简单处理公式
...
def compute_sharpe_ratio(net_values):
    ...
    #用日线数据算出来的方差是日方差var，var*245 = 年化方差，年化标准差 = pow（年化方差，1/2）

    # 年化收益
    annual_profit = compute_annual_profit(trading_days, net_values[-1])

    # 夏普比率
    sharpe_ratio = (annual_profit - 4.75) / (profit_std * pow(245, 1 / 2)) #Rf = 4.75%
