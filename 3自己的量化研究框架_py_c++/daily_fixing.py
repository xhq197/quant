...
def fill_daily_k_at_suspension_days(begin_date=None, end_date=None):
    ...
    while 1:
        ...
        basic_cursor = DB_CONN['basic'].find(
            ...
            # 填充时需要用到两个字段股票代码code和上市日期ipoDate，
            # 上市日期用来判断
            projection={'code': True, 'ipoDate': True, '_id': False},
...
def fill_daily_k_at_suspension_days_at_date_one_collection(
    ...
    for date in all_dates:
        ...
        for basic in basics:
            code = basic['code']
            # 如果循环日期小于
            if date < basic['ipoDate']:
                # print('日期：%s, %s 还没上市，上市日期: %s' % (date, code, basic['ipoDate']), flush=True)
                pass
            else:
                # 找到当日数据
...
if __name__ == '__main__':
    #填充复权因子和前收盘价
    fill_au_factor_pre_close('2015-01-01')
    #停牌日填充一交易日数据
    #增加is_trading字段，区分停牌日和交易日
    #补充停牌日的日K数据，根据当前数据现状，填充的数据为：open、
    # close、high、low为停牌前最后一个交易日的close，volume为0，
    # is_trading为false。
    # fill_is_trading_between('2020-12-31')
    # fill_daily_k_at_suspension_days('2015-01-01')
